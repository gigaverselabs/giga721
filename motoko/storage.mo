...
shared(msg) actor class Storage(_owner: Principal) {
    ...
    public shared(msg) func setOwner(owner: Principal) : async Bool {
        assert(msg.caller == owner_);
        owner_ := owner;
        return true;
    };
    ...
    public shared(msg) func uploadHistory(data: [OpRecord]) : async Bool {
        assert(msg.caller == owner_);
        
        ops := Array.thaw(Array.append(Array.freeze(ops), data));
